...
# Custom CSS - CLEANED UP (Removed square border from template section)
st.markdown("""
    <style>
    /* 1. Main Background */
    ...
    /* 5. DARK GREY BUTTONS (Standardized) */
    ...
    /* 6. Simple Template Container (No border/square) */
...
def load_model_assets():
    try:
        ...
        if isinstance(loaded_object, dict):
            ...
            # Fallback for direct pickle files
...
if page == "Main Analysis":
    ...
    # Column Layout for Download and Upload
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("1. Download Format")
        # Template Section (CLEAN - Square removed)
        st.markdown('<div class="clean-template-section">', unsafe_allow_html=True)
        if feature_list is not None:
            # Create template based on model features
            ...
            st.warning("Please ensure 'gbm_diagnostic_model-1.pkl' is in the root folder to generate the template.")
    ...
    if uploaded_file and model:
        ...
        mcol2.markdown(f"**Classification:** <span style='color:{color}; font-weight:bold;'>{status}</span>",
                       unsafe_allow_html=True)
        st.progress(prob)
...
# --- 5. PAGE: DOCUMENTATION ---
    ...
    st.write("Information regarding model training and feature alignment.")
...
# --- FOOTER ---
st.markdown("---")
st.caption("Experimental Multi-Omic Analysis Tool | XGBoost Diagnostics")
